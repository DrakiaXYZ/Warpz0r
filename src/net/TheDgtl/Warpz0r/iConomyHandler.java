...
public class iConomyHandler {
	...
	public static double getBalance(String player) {
		if (useiConomy && register != null) {
			...
			if (acc == null) {
				Warpz0r.log.info("[" + pName + "::ich::getBalance] Error fetching Register account for " + player);
				return 0;
			}
			return acc.balance();
		}
	...
	public static boolean chargePlayer(String player, String target, double amount) {
		if (useiConomy && register != null) {
			...
			MethodAccount acc = method.getAccount(player);
			if (acc == null) {
				Warpz0r.log.info("[" + pName + "::ich::chargePlayer] Error fetching Register account for " + player);
				return false;
			}
